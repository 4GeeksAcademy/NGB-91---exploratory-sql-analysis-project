...
JOIN regins ON observations.regions_id = regions.id 
...
FROM observations
JOIN species ON oberservation.species_id = species.id
GROUP BY species.id
...
SELECT 
    regions.name,
    regions.country,
    COUNT(DISTINCT observation.species_id) AS species_count
FROM observations
...
FROM observations
JOIN species ON obeservation.species_id = species.id
GROUP BY species.id
...
SELECT  
    ...
    COUNT(*) AS total
FROM observations
GROUP BY oberservation.observer
ORDER BY total DESC
LIMIT 10;
